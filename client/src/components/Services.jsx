...
const services = [
  ...
  {
    num: '02',
    icon: <svg viewBox="0 0 24 24"><path d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.66 0 3-4.03 3-9s-1.34-9-3-9m0 18c-1.66 0-3-4.03-3-9s1.34-9 3-9"/></svg>,
...
export default function Services() {
  useEffect(() => {
    ...
    document.querySelectorAll('.scard').forEach(card => {
      const ico = card.querySelector('.sico')
      const num = card.querySelector('.snum')
      const tl = gsap.timeline({ paused: true })
        .to(card, { y: -10, boxShadow: '0 28px 72px rgba(14,25,30,.13)', duration: .35, ease: 'power2.out' }, 0)
        .to(ico,  { scale: 1.14, rotation: -6, duration: .35, ease: 'back.out(2)' }, 0)
        .to(num,  { opacity: .28, scale: 1.06, duration: .35, ease: 'power2.out' }, 0)
